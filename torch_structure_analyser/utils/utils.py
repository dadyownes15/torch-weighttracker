...
from ..analysis.types import StructureInspectionReport
...
def format_structure_summary(report: StructureInspectionReport) -> str:
    rows = [
        [
            row.group_id,
            row.axis.value,
            row.root_module_name,
            str(row.structure_count),
            str(row.coupled_member_count),
            str(row.total_touched_params),
            _format_count_range(row.touched_params_min, row.touched_params_max),
            _format_count_range(row.touched_tensors_min, row.touched_tensors_max),
        ]
        for row in report.summary_rows
    ]
    lines = [
        "StructureSummary",
        f"  model_params={report.model_total_params}",
        _render_table(
            [
                "group_id",
                "axis",
                ...
                "structures",
                "members",
                "total_touched_params",
                "touched_params_per_structure",
                "touched_tensors",
            ],
            rows,
        ),
    ...
    for row in report.summary_rows:
        if len(row.coupled_member_names) == 0:
            continue
        lines.append(f"  members for {row.group_id}:")
        for member_name in row.coupled_member_names:
            lines.append(f"    - {member_name}")
    return "\n".join(lines)


def _format_count_range(min_value: int, max_value: int) -> str:
